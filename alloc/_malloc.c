  ...
  */
  ...
  if (MORECORE_CONTIGUOUS && !use_noncontiguous(m)) {
    ...
    if (ss == 0) {  /* First time through or recovery */
      ...
      asize = granularity_align(nb - m->topsize + SYS_ALLOC_PADDING(m->modalign));
    ...
    if (tbase == CMFAIL) {    /* Cope with partial failure */
      if (br != CMFAIL) {    /* Try to use/extend the space we did get */
        if (asize < HALF_MAX_SIZE_T &&
            asize < nb + SYS_ALLOC_PADDING(m->modalign)) {
          size_t esize = granularity_align(nb + SYS_ALLOC_PADDING(m->modalign) - asize);
  ...
  if (HAVE_MMAP && tbase == CMFAIL) {  /* Try MMAP */
    size_t rsize = granularity_align(nb + SYS_ALLOC_PADDING(m->modalign));
  ...
  if (HAVE_MORECORE && tbase == CMFAIL) { /* Try noncontiguous MORECORE */
    size_t asize = granularity_align(nb + SYS_ALLOC_PADDING(m->modalign));
    if (asize < HALF_MAX_SIZE_T) {
      char* br = CMFAIL;
      char* end = CMFAIL;
      ACQUIRE_MALLOC_GLOBAL_LOCK();

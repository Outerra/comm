...
/*
  ...
  modalign specifies desired modulo of chunk alignment, default 0
*/
...
static void do_check_mmapped_chunk(mstate m, mchunkptr p) {
  ...
  size_t len = (sz + m->modalign + (p->prev_foot & ~IS_MMAPPED_BIT) + MMAP_FOOT_PAD);
  assert(is_mmapped(p));
  assert(use_mmap(m));
  assert((is_aligned(chunk2mem(p), m->modalign)) || (p->head == FENCEPOST_HEAD));
  assert(ok_address(m, p));
  ...
  */
  ...
  if (MORECORE_CONTIGUOUS && !use_noncontiguous(m)) {
    ...
    if (ss == 0) {  /* First time through or recovery */
      ...
      if (base != CMFAIL) {
        asize = granularity_align(nb + TOP_FOOT_SIZE(m->modalign) + SIZE_T_ONE);
      ...
      asize = granularity_align(nb - m->topsize + TOP_FOOT_SIZE(m->modalign) + SIZE_T_ONE);
    ...
    if (tbase == CMFAIL) {    /* Cope with partial failure */
      if (br != CMFAIL) {    /* Try to use/extend the space we did get */
        if (asize < HALF_MAX_SIZE_T &&
            asize < nb + TOP_FOOT_SIZE(m->modalign) + SIZE_T_ONE) {
          size_t esize = granularity_align(nb + TOP_FOOT_SIZE(m->modalign) + SIZE_T_ONE - asize);
  ...
  if (HAVE_MMAP && tbase == CMFAIL) {  /* Try MMAP */
    size_t req = nb + TOP_FOOT_SIZE(m->modalign) + SIZE_T_ONE;
  ...
  if (HAVE_MORECORE && tbase == CMFAIL) { /* Try noncontiguous MORECORE */
    size_t asize = granularity_align(nb + TOP_FOOT_SIZE(m->modalign) + SIZE_T_ONE);
    if (asize < HALF_MAX_SIZE_T) {
      char* br = CMFAIL;
      char* end = CMFAIL;
      ACQUIRE_MORECORE_LOCK();
...
static size_t release_unused_segments(mstate m) {
  ...
  while (sp != 0) {
    ...
    if (is_mmapped_segment(sp) && !is_extern_segment(sp)) {
      ...
      if (!cinuse(p) && (char*)p + psize >= base + size - TOP_FOOT_SIZE(m->modalign)) {

...
private:
    ...
    typedef hashtable<VAL,KEY,HASHFUNC,EQFUNC,GETKEYFUNC,ALLOC>  _Self;

protected:

    typedef hashtable<VAL,KEY,HASHFUNC,EQFUNC,GETKEYFUNC,ALLOC>   _HT;
...
protected:
    ...
    Node* find_node( const KEY& k ) const
    {
        ...
        while(n)
        {
            if( _EQFUNC( k, _GETKEYFUNC(n->_val) ) )
                return n;
            n = n->_next;
        }
        return 0;
    ...
    uints del( const KEY& k )
    {
        ...
        uints c=0;
        while( n  &&  _EQFUNC( k, _GETKEYFUNC(n->_val) ) )
        {
            Node* t = n->_next;
            delete n;
            n = t;
...
public:
    ...
    std::pair<iterator, iterator> equal_range( const KEY& k )
    {
        ...
        Node* l = f->_next;
        while( l  &&  _EQFUNC( k, _GETKEYFUNC(l->_val) ) )
            l = l->_next;

...
inline constexpr uints align_to_chunks( uints uval, uints usize )
{ return uints((uval+usize-1)/usize); }

/// Aligns value to given chunk size (enlarges to next chunk boundary)
inline constexpr uint64 align_to_chunks64( uint64 uval, uint64 usize )
{ return (uval+usize-1)/usize; }


/// Aligns value to nearest multiplier of 2 pow rsize chunk size
inline constexpr uints align_value_to_power2( uints uval, uchar rsize )
{ return uints((uval+(uints(1)<<rsize)-1) &~ ((uints(1)<<rsize)-1)); }

/// Aligns value to nearest multiplier of 2 pow rsize chunk size
inline constexpr uint64 align_value_to_power2_64( uint64 uval, uchar rsize )
{ return (uval+(uint64(1)<<rsize)-1) &~ ((uint64(1)<<rsize)-1); }


/// Aligns value to given chunk size (enlarges to next chunk boundary)
inline constexpr uints align_to_chunks_pow2( uints uval, uchar rsize )
{ return uints((uval+((uints(1)<<rsize)-1))>>rsize); }

/// Aligns value to given chunk size (enlarges to next chunk boundary)
inline constexpr uint64 align_to_chunks_pow2_64( uint64 uval, uchar rsize )

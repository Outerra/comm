...
public:
    ...
    opcd read_binary(token& tok, binstream_container_base& c, uints n, uints* count, const void* defval)
    {
        uints nr = n;
        if (c.is_continuous() && n != UMAXS)
            nr = charstrconv::hex2bin(tok, c.insert(n, defval), n, ' ');
        else {
            for (; nr > 0; --nr)
                if (charstrconv::hex2bin(tok, c.insert(1, defval), 1, ' ')) break;
        }

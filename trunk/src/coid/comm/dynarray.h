...
public:
    ...
    ///Debug checks
#ifdef _DEBUG
# define DYNARRAY_CHECK_BOUNDS_S(k)          check_bounds((ints)k);
# define DYNARRAY_CHECK_BOUNDS_U(k)          check_bounds((uints)k);
#else
# define DYNARRAY_CHECK_BOUNDS_S(k)
# define DYNARRAY_CHECK_BOUNDS_U(k)
#endif

    void check_bounds(ints k) const         { DASSERTE( k>=0 && (uints)k<_count(), ersOUT_OF_RANGE ); }
    void check_bounds(uints k) const        { DASSERTE( k<_count(), ersOUT_OF_RANGE ); }
